...
run "test_basic_pool" {
  ...
  assert {
    condition     = module.dev_center_project_pools["pool1"] != null
    error_message = "Pool should exist"
  }
}

// Test for pool with custom properties
run "test_custom_pool" {
  command = plan
  ...
  variables {
    dev_center_project_pools = {
      custom_pool = {
        ...
        dev_box_definition_name                 = "definition1"
        display_name                            = "Custom Pool with Special Settings"
        local_administrator_enabled             = true
        network_connection_name                 = "custom-network"
  ...
  assert {
    condition     = module.dev_center_project_pools["custom_pool"] != null
    error_message = "Custom pool should exist"
  }
}

// Test multiple pools
run "test_multiple_pools" {
  command = plan
  ...
  assert {
    condition     = module.dev_center_project_pools["pool1"] != null
    error_message = "Pool1 should exist"
  }

  assert {
    condition     = module.dev_center_project_pools["pool2"] != null
    error_message = "Pool2 should exist"
  }
}
